...
//prototype of function declaration on the other file, McodeMod.c
//extern char * mcodestring(int);

//take command line arguments, ./MorseCode -w WordToDisplay
//int main(int argc, const char * argv[]) {

    //if the default param set is not set then return -1 for error
    if (argc > 3 || argc < 3) {
        printf("You have provided too many inputs. Closing program");
        return -1;
    }
        
    //if the default param set is achieved then proceed with program development
    else if (argc == 3){
        if (argv[2] != NULL && strcmp(argv[1], "-w")) {
